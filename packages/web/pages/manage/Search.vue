...
const search = async ()=>{
  ...
  if (time>0){
    ...
    googleSearch({keyword:`${searchForm.value.keyword} LTD site:UK inurl:team`,page:searchPage.value.page,num:searchPage.value.num}).then((res)=>{
      if (res.status===200){
...
<!--    </el-select>-->
  ...
  <div v-else>
<!--    <img class="w-full h-[500px]" src="~/assets/map.jpg" alt="">-->
    111
  </div>
  </div>
</template>